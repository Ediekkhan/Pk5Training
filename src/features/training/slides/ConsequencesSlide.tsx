...
export function SlideConsequencesModule({
  ...
  if (view === "overview") {
    return (
      <div>
        <SlideHead
          kicker="Chapter 7 · Interactive Module"
          title="Consequences of confidentiality breaches"
          sub="Explore each consequence — in any order. All 10 must be viewed to continue."
        />
          ...
          {CONSEQUENCE_TOPICS.map((c, idx) => {
            ...
            return (
              <motion.button
                ...
                className={`group relative overflow-hidden rounded-2xl border p-4 text-left shadow-soft transition-all hover:-translate-y-0.5 hover:shadow-elevated ${
                  done
              ...
              >
                ...
                <div
                  className={`mb-3 grid size-10 place-items-center rounded-xl ${
                      ...
                      : "bg-primary-soft text-gold-dark group-hover:gold-gradient group-hover:text-white transition-all"
                  }`}
                >
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="text-xs font-semibold text-muted-foreground">
          Consequence <span className="text-charcoal">{idx + 1}</span> of{" "}
          {CONSEQUENCE_TOPICS.length}
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="mt-1 text-[11px] text-muted-foreground">
          {viewed.size} of {CONSEQUENCE_TOPICS.length} explored
        </div>
      </div>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border p-5">
          ...
          <Button
            size="sm"
            onClick={() => goTopic(idx + 1)}
            disabled={idx === CONSEQUENCE_TOPICS.length - 1}
          >
